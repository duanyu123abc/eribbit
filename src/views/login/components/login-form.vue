      ...
      <template v-if="!isMsgLogin">
        ...
        <div class="form-item">
          <div class="input">
            ...
            <Field autocomplete="off" :class="{error:errors.password}" v-model="form.password" name="password" type="password" placeholder="请输入密码" />
          </div>
      ...
      <template v-else>
        ...
        <div class="form-item">
          <div class="input">
            ...
            <Field :class="{error:errors.code}" name="code" v-model="form.code" type="code" placeholder="请输入验证码" />
...
export default {
  ...
  setup () {
    ...
    // 3.定义Field的name属性指定的校验规则函数，Form的validation-schema接收定义好的校验规则，是个对象
    // 4.自定义组件需要校验 必须支持v-model 然后Field使用AS指定为组件名称
    ...
    watch(isMsgLogin, () => {
      ...
      form.mobile = null
      form.code = null
      // 如果是没有销毁Field组件，之前的校验结果不会被清除，例如：v-show切换的
      // Form 组件提供了一个resetForm 函数清楚校验结果
    ...
    // 需要再点击的时候对整体表单进行校验
    const login = async () => {
      // Form提供了一个validate函数作为整体表单校验，返回的是一个promise
      ...
      if (valid) {
        try {
          ...
          // 失败提示
          if (e.response.data) {
            Message({ type: 'error', text: e.response.data.message || '用户名或者密码错误' })
          }
        }
      }
    }
